...
func (a *app) readString(qs url.Values, key string, defaultValue string) string {
	string := qs.Get(key)
	if string == "" {
		return defaultValue
	}
...
func (a *app) readCSV(qs url.Values, key string, defaultValue []string) []string {
	csv := qs.Get(key)
	if csv == "" {
		return defaultValue
	}
...
func (a *app) readInt(qs url.Values, key string, defaultValue int, v *validator.Validator) int {
	string := qs.Get(key)
	if string == "" {
		return defaultValue
	}
